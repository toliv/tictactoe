...
def test_user_error(client):
    ...
    get_response = client.get(
        "/users", query_string={"other_param": response.json["id"]}
    )
...
def test_join_game(client):
    ...
    join_response = client.post(
        f"/games/{response.json['id']}/join", data={"user_id": user.json["id"]}
    )
    assert join_response.status_code == 200
    # Joining again should be a failure
    join_again_response = client.post(
        f"/games/{response.json['id']}/join", data={"user_id": user.json["id"]}
    )
    ...
    other_join_response = client.post(
        f"/games/{response.json['id']}/join", data={"user_id": other_user.json["id"]}
    )
...
def test_play_game(client):
    ...
    r = client.post(
        f"/games/{game.json['id']}/join", data={"user_id": other_user.json["id"]}
    )
    assert r.json["status"] == "in_progress"
    # Make a move
    move_response = client.post(
        f"/games/{game.json['id']}/move",
        data={
    ...
    assert {"result": "win", "user_id": user.json["id"]} in move_response.json[
        "results"
    ]
    assert {"result": "loss", "user_id": other_user.json["id"]} in move_response.json[
        "results"
    ]
